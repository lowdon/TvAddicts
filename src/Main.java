...
public class Main {
	...
	private static void addCharacter(Scanner in, TvAddicts addicts) {
		...
		try {
			if (kind.equals("real")) {
				...
				System.out.println(character.name() + " is now parte of " + character.show() + ". This is " + ((RealCharacter)character).actor().name() + " role " + ((RealCharacter)character).actor().rolesNumber() + ".");
			}
			else if (kind.equals("real")) {
				Character character = addicts.addVirtualCharacter(characterName, actorOrCompanyName, feeOrCost);
				System.out.println(character.name() + " is now parte of " + character.show() + ". This is  a virtual actor.");
			}
			else {
	...
	private static void addRelationship(Scanner in, TvAddicts addicts) {
		...
		try {
			...
			System.out.println(addicts.character(character1Name).name() + "has now " + addicts.character(character1Name).kidsNumber() + " kids. " + addicts.character(character2Name).name() + " has now " + addicts.character(character2Name).parentsNumber() + " parent(s).");
		}
		catch (NoShowSelectedException exception) {
			 System.out.println("No show is selected!");
		}
